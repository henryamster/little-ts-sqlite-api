...
export class Logger {
  ...
  log(
    eventOrType: LogEvent | LogEventType,
    level?: LogEventLevel,
    message?: string,
    context?: any,
    stackTrace?: string
    ...
    if (eventOrType instanceof LogEvent) {
      ...
      this.emit(
        eventOrType,
        new LogEvent(eventOrType, level!, message, context, stackTrace)
      );
    }
  }
...
export function LogEventDecorator(
  type: LogEventType,
  level: LogEventLevel,
  message?: string,
  context?: any,
...
class LogEventPrettyPrinter {
  static print(event: LogEvent) {
    ...
    const prettyMessage = `${timestamp.toISOString()} [${type}] [${level}] ${message} ${
      context ?? ""
    } ${stackTrace ?? ""}`;
